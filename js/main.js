(function ($) {
    ...
    $(window).scroll(function () {
        if ($(window).width() < 992) {
            ...
            if ($(this).scrollTop() > 45) {
                $('.fixed-top').addClass('bg-light ').css('top', -45);
            } else {
                $('.fixed-top').removeClass('bg-light ').css('top', 0);
            }
        }
    });
...
const initSlider = () => {
    ...
    const autoPlaySpeed = 2500; // Speed in milliseconds
    let autoPlayInterval;

    // Handle scrollbar thumb drag
    scrollbarThumb.addEventListener("mousedown", (e) => {
        clearInterval(autoPlayInterval); // Stop auto-play on manual interaction
        ...
        const handleMouseUp = () => {
            ...
            startAutoPlay(); // Resume auto-play after manual interaction
        }
    ...
    startAutoPlay(); // Start the auto-play when the slider is initialized
}
